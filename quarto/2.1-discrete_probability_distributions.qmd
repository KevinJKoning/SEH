...
### Named Probability Distributions are Special

We intentionally introduced *probability distributions* as models of a data generating process to demystify them and quickly introduce some examples. However, it's worth slowing down for a moment to understand what *probability distributions* are in general, and what makes the named probability distributions statisticians use so frequently special.

#### Probability Distributions

In words, we can intuitively describe a data generating process that has a distribution of outcomes, such as 'the sum of three dice when rolled out of a cup'. This glosses over some aspects a statistician would want to call out explicitly:

-   There is some *sample space* that has possible *outcomes.* For example, if the three dice are rolled out of the cup a possible outcome is they show a 2, 5, and 3. A roll of 9, -1, and 4 is not in the *sample space.*

-   A *random variable* describes how an outcome is translated into a numeric value. Our examples sum the dice, but a different random variable of the same outcome could count how many times a dice shows a 4.

-   Finally, if we determine how often values of the *random variable* appear, we get a *probability distribution.*

Again, we say that more intuitively by stating that we have a data generating process that has a distribution of outcomes. This process does not need to be something that is amenable to a nice closed-form analytic solution.

#### Named Probability Distributions

The named probability distributions are special cases that have been very useful to statisticians.

-   They describe a common event.

-   They can be succinctly described with mathematical expressions, which are frequently closed-form analytic solutions, but not always.

-   ...

However, the named probability distributions do hold a very special place in statistics. (we refer to n

**We should have something that goes back to using computation to approximate probabilities, probably with the binomial distribution, and also have that smooth our way into probability of multiple events**

::: callout-important
The introduction to probability distributions attempted to demystify them before they even became mystical... However, they hold an beguiling place of importance in statistics. Why probability distributions holds such a special place in statistics I think is very confusing to engineers. I think it's important to consider them as just a subset of all possible models of data generating processes, however, they have properties that makes them an extremely useful subset:

-   Describe common processes

-   Reverse probability

-   Describe 'meta' processes'

-   (Later will describe uncertainty)

We can think of probability distributions as a special cases of models that have an analytic/closed form solution for the 'inverse' calculation of going backwards from the data they generate to probabilities. The real world, however, doesn't fit into nice analytic solutions, which is why we lean so heavily on sampling/bootstrapping. But that doesn't mean there will not be circumstances in which easily calculating the probability from data is not useful. Ultimately, and especially with errors, we will find probability distributions to be useful...
:::
