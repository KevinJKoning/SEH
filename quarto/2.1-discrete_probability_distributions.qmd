...
A conditional probability distribution is one in which we are given the state of one or more variables. For example, let's examine P(Breakdown, Extreme_Heat \| Brand = BrandB), which in words is the joint probability distribution of Breakdown and Extreme_Heat given that Brand = BrandA.
...
Because BrandA is a constant, we could just write it this way:
...
It is also possible to have a probability distribution from one of the *named* probability distributions in which we have a set of values for the parameters, instead of one value. For example, we can have a Binomial distribution where $P(k, \quad n \in {(3, 4),} \quad p \in {(0.2, 0.3)})$. This is like the 'Breakdown' table where $P(Breakdown, \quad ExtremeHeat \in {(Yes, No),} \quad Brand \in {(BrandA, BrandB)})$
