...
In more statistical terms, this chapter utilizes a **Bayesian** perspective. We will introduce Bayes Theorem later... There are two key
...
The simplest example of 1) is flipping a coin that, upon basic description or examination, appears likely to be fair (produces 50% heads). However, let's say the first three flips are: Heads, Heads, and Heads. Now what should we estimate the true propability of heads to be?

- In the frequentist view, the probability with the maximum likelihood is simply P=1 for heads.
- In the bayesian view, P of heads is now greater than 0.5 and less than 1, depending on how strongly you had a prior belief that the coin was fair.

If you knew the coin had just come from the bank, you could give a strong prior, that even after the three heads, the probability would be only slightly above 0.5. However, if you didn't know the origin of the coin, you may have a much weaker prior - but you also know it would be virtually impossible to create a coin that could only land heads - and therefore you would still give a prior that prevented P=1.
...
app_ui = ui.page_fluid(
    ...
    ui.layout_sidebar(
        ui.sidebar(
            ...
            ui.input_slider("prior_sd", "Prior Standard Deviation", min=0.01, max=0.2, value=0.1, step=0.01),
...
def server(input, output, session):
    ...
    @reactive.effect
    def handle_flip():
        input.flip()  # Create dependency on button click
        
        # Reset history when parameters change
        if len(flips_history.get()) > 0:
            if input.true_prob() != flips_history.get()[-1]["true_prob"] or \
               input.prior_sd() != flips_history.get()[-1]["prior_sd"]:
                flips_history.set([])
        ...
        # Store the results with cumulative counts
        current_history = flips_history.get()
        
        # If this is the first flip sequence or parameters changed
        if len(current_history) == 0:
            ...
            current_history.append({
                "num_flips": n,
                "total_flips": prev["total_flips"] + n,
                "heads_count": heads_count,
                "total_heads": prev["total_heads"] + heads_count,
                "true_prob": true_prob,
                "prior_sd": input.prior_sd()
            })
        
        flips_history.set(current_history)
    ...
    def posteriorPlot():
        ...
        if len(flip_counts) > 1:
            axins = fig.add_axes([0.6, 0.2, 0.35, 0.25])
            axins.plot(flip_counts, posterior_values, 'ko-', ms=4)
            axins.set_xlabel('Number of Flips')
            axins.set_ylabel('Posterior Density at True p')
            axins.set_title('Evolution of Posterior')
