...
Something you hopefully noticed in the app was that after flipping coins the chart showed a 'posterior' distribution. This distribution shows what we believe about the probability a random flip shows heads after we've seen a number of flips. We may say the posterior is a revised belief after witnessing additional evidence/data.
...
This description was in reference to the model before we considered any data - we want to update things after we've seen data, which is an different world called the posterior.

Here's an updated coin flipping app to try to make this more obvious:
...
def server(input, output, session):
    ...
    def fixedPHistograms():
        ...
        fig.suptitle(f'Histograms with P Values from Prior Distribution ({n} flips each)', fontsize=16)
        plt.tight_layout()
        return fig
    ...
    def posteriorPHistograms():
        ...
        fig.suptitle(f'Histograms with P Values from Posterior Distribution ({n} flips each)', fontsize=16)
        plt.tight_layout()
        return fig
...
#### Observations About the Posterior

This is another distribution that usually moved
