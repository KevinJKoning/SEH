...
What's learned with linear regression can be extended to many datasets that are not linear, but can be transformed to *look* linear. Typically the transformation will also assume that the variability $\epsilon$/$\sigma$ follows another distribution. In linear regression the assumption is a normal distribution, but transformations can assume, for example Binomial, Poisson, or Logistic distributions.
...
TODO: x_i notation instead of X notation.

$$
\ln \left( \frac{P(Y = 1 \mid X)}{1 - P(Y = 1 \mid X)} \right) = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \dots + \beta_n X_n
$$
...
- $P(Y = 1 \mid X)$ is the probability that the dependent variable $Y$ equals 1 given the input features $X$.
- $\beta_0$ is the intercept term and $\beta_1, \beta_2, ..., \beta_n$ are the coefficients for the independent variables $X_1, X_2, ..., X_n$.
    ...
    ui.layout_sidebar(
        ...
        ui.navset_tab(
            ui.nav_panel("Linear Regression",
                ui.output_plot("linearPlot", height="400px"),
                ui.br(),
                ui.output_text("linear_accuracy"),
            ),
            ui.nav_panel("Logistic Regression",
                ui.output_plot("logisticPlot", height="400px"),
                ui.br(),
                ui.output_text("logistic_accuracy"),
            ),
        ),
    ),
)
...
def server(input, output, session):
    ...
    def generate_data():
        ...
        z = separation * X.ravel() + np.random.normal(0, 0.5, n)
        prob = 1 / (1 + np.exp(-z))
    ...
    def description():
        return (
            f"Data Generation Model:\n\n"
            f"P(y=1) = 1 / (1 + exp(-({input.separation():.1f}x + ε)))\n"
            f"where ε ~ N(0, 0.5)"
        )
    ...
    def linear_accuracy():
        ...
        lr = LinearRegression()
        lr.fit(X, y)
        y_lr_pred = (lr.predict(X) > 0.5).astype(int)
        acc = accuracy_score(y, y_lr_pred)
        return f"Accuracy Score: {acc:.3f}"
...
We chose to include logistic regression as it is a preview to a piece of the Bayesian Neural Networks we tackle in the next chapter.

### Bespoke/Bayesian Models

TODO.

When we added the non-constant error term to our linear regression model, we headed off the well-trod path of statistical models with analytical solutions. Heading off that well-trod path often leads to another reasonably well-formed path in Bayesian statistics and causal modeling. We intentionally avoid framing our work as Frequentist or Bayesian, but here are a couple key elements we want to include as we move toward models without analytical solutions:

* Causal diagrams. These can have more technical uses, but at the very least they communicate our model intent.
* Bayesian solutions to P(M|D). Without analytical solutions, we need another way. This will wait until the second half of the primer.

#### Causal Diagrams

As mentioned, a causal diagram is a multi-use tools, but at the very least they will communicate our intent, and that often tends to also hone/refine thinking...

Here's what we want in a diamond model:
```{mermaid}
graph LR
...
This clearly shows that we intend cut, color, clarity, and size to be used in a linear regression model for price, that size will also be used for a carat threshold model for price, and finally that size will also be used to model the variance for price.

The structure of the diagram, i.e. the presence of *colliders*, *forks*, *backdoor paths* etc. can give clarity on dependence, independence, spurious correleations , and such, but are a much more advanced topic.
...
Again, if we look back at the sale prices, in a fairly small number of sales at that price, several are above $12,000, so these estimates again seem terrible. Here, the estimate of the averagesale price was somewhat low, but also now due to an *underestimate* of the standard deviation at 1.55 carats.
...
TODO: Probability of data for small and large carats, show how it seems more reasonable.
Worth the space???
