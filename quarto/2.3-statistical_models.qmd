...
Previously our models only had parameters/settings, but now we use models that can also ingest data. We start with the most common generalized linear model, linear regression. Just like in previous chapters, we focus on generating data to understand our models. This will allow us to quickly see the result of the underlying model assumptions that we would have been likely to miss otherwise.

We focus on errors and understanding how probable an error is. This is what makes our models fundamentally statistical. Different models will have errors with different distribution, and it's important we get the probability of our errors correct if we want accurate models. We will get to take advantage of what we learned in the previous two chapters.
...
Up to this point we have been using models that require only *parameters* to simulate a data generating process. We can think of parameters as the settings of the model - examples of parameters we chose earlier were the number of dice to roll at a time, or the mean and variance of a probability distribution.
...
The single variable linear regression model has this mathematical form:

$$
y_i = \beta_0 + \beta_1 x + \epsilon
$$

Where $\beta_0$ is the y-intercept, $\beta_1$ is the the slope, and $\epsilon$ is the error. In the diamond model the $x$ variable is the diamond size/carat and $y$ is the price.

#### Testing Inverse Link with Gamma Distribution

A long running theme is to just use computation instead of complex analytical solutions... But since those can be elegant, we'll use them when appropriate... Here:
...
app_ui = ui.page_fluid(
    ui.h2("Gamma GLM Diamond Price Modeling"),
    ui.layout_sidebar(
        ui.sidebar(
            ui.input_slider("num_points", "Number of Simulated Points", 
                           min=100, max=1000, value=200, step=100),
            ui.input_slider("shape", "Gamma Shape Parameter", 
                           min=0.01, max=20, value=10, step=1),
            ui.input_slider("slope", "Slope", 
                           min=-0.001, max=0.001, value=0.0005, step=0.0001),
            ui.input_slider("intercept", "Intercept", 
                           min=0.000001, max=0.001, value=0.0005, step=0.0001),
            ui.input_checkbox("show_original", "Show Original Data", True),
            ui.input_checkbox("show_simulated", "Show Simulated Data", True),
            width=250
        ),
...
def server(input, output, session):
    ...
    def generate_simulated_data():
        ...
        # Calculate mean using inverse link function
        # μ = 1/(β₀ + β₁x)
        mean = 1 / (input.intercept() + input.slope() * X_new)
        
        # Generate gamma distributed random variables
        shape = input.shape()
        scale = mean / shape  # Scale parameter to achieve desired mean
        
        Y_new = gamma.rvs(shape, scale=scale)
    ...
    def diamondPlot():
        ...
        if input.show_simulated():
            ...
            # Add the mean curve
            X_line = np.linspace(diamonds['carat'].min(), diamonds['carat'].max(), 100)
            Y_line = 1 / (input.intercept() + input.slope() * X_line)
            ax.plot(X_line, Y_line, 'r--', 
                   label=f'μ = 1/({input.intercept():.4f} + {input.slope():.4f}x)')
        
        ax.set_title('Diamond Carat vs Price: Gamma GLM with Inverse Link')
        ax.set_xlabel('Carat')
        ax.set_ylabel('Price')
        ax.grid(True)
        ax.legend()
