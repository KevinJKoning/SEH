...
Let's also remember that the variability we describe with statistics has two sources, Aleatoric Uncertainty (Randomness) and Epistemic Uncertainty (Lack of Knowledge). When we simulate data from 'real life' with just a probability distribution, we are lumping both Aleatoric and Epistemic Uncertainty together. However, we now start to use models that incorporate data to reduce the Epistemic Uncertainty.

## Preview

Previously we had models based on outcomes from the whole sample space, now we look at estimates conditioned on data. This conditioning essentially eliminates a part of the sample space, allowing us to 'zoom in' on another area of interest. However, variability still exists, and it depends on the problem whether the shape of the uncertainty has been modified by the conditioning.
...
**WE DON'T FOCUS ON LINEAR REGRESSION PER SE TO START, WE FOCUS ON THE ERRORS FROM LINEAR REGRESSION FIRST**

We start with linear regression and approach it in a way that is likely backwards from your previous introduction (and we promise you'll learn something new). Typically you'll be presented with some data and the task will be to find a best fit line through the data. Here we will think about it in the other direction, if we already have a linear regression model, what data does it generate? And how likely were we to see that data once generated?
...
The single variable linear regression model has this classical mathematical form:

$$
y_i = \beta_0 + \beta_1 x_i + \epsilon
$$

Where $\beta_0$ is the y-intercept, $\beta_1$ is the the slope, and $\epsilon$ is the error. In the diamond model the $x_i$ variable is a diamond size/carat and $y_i$ is the price.

We'd like to take this opportunity to nitpick the classical form, as it implies that we are modeling a deterministic process, and that variations from that deterministic process constitute 'errors'. A more statistically genuine way to think about linear regression is:

$$
y_i \sim \mathcal{N}(\beta_0 + \beta_1 x_i, \sigma^2)
$$

**Explanation of notation**...Which indicates the predicted y value is actually normally distributed around a mean value based on the $\beta$ coefficient and the x value, with variance $\sigma^2$.

(Note that in real world statistics, we generally cannot seperate our lack of knowledge from genuine randomness. If we could, then error to describe lack of knowledge may be more appropriate.)

Another view to really start getting at the reality of the situation is this:

$$
P(Y = y \mid X, \beta, \sigma)
$$

Which indicates that the probability of the y value is *conditioned* on $X, \beta$ and $\sigma$. Before we added the data/X component, we just conditioned on the parameter values, $\beta$ and $\sigma$.

#### An Aside on Conditional Dependence

Let's say we drive our car and we record a table of average gas mileage:

| Miles Per Hour | Measured Miles Per Gallon (Sorry Metric System) | Standard Deviation |
|-----------------|---------------------------------------|-----------------|
| 30 | 32 | 2 |
| 45 | 36 | 2 |
| 60 | 37 | 2 |
...
It's important to note that the average miles per gallon will have some expected variability due to items like traffic, wind speed/direction, stopping frequency, etc. Let's assume that variability is normally distributed with a standard deviation of 2 miles per gallon. Let's say you also just left the recording of gas mileage on for all speeds, and you got an average of 31 mpg with a standard deviation of 5 miles per gallon.

The 'Grand Mean' has a mean value and variation - this is very similar to the probability distributions we reviewed in previous chapters, we had no other information and described our data generating process with just a named probability distribution and its parameters. However, now that we have the data, we can also get a conditional probability, $P(GasMileage \mid Speed)$, which stated out loud is the 'probability of gas mileage given speed'. To be complete we should include all the variables, $P(GasMileage \mid Speed, MeasuredMPG, \sigma)$.

To describe mileage at a specific speed, we can then write something like this $P(GasMileage \mid Speed=45)$, where we go back to the shorthand because we are implicitly referencing the table. Now we can look at the table and see at 45 Miles per Hour we measured 36 miles per gallon. Let's point out that we are not ignorant enough to believe if we drove at 45 miles per hour again, that we would get exactly 36 miles per gallon - we expect to get *about* 36 miles per gallon, give our take a few based on the probabilities of the normal distribution.
...
Here's our new mathematical model:

$$
y_i = \beta_0 + \beta_1 x + x \epsilon
$$

Here's the new app (it seems to be taking a very long time to load due to the extra libraries required to solve the new model):
...
That fits the change in variance with price much better, although it could still use some improvements. I'd like to stress that very few people would have noticed the flaw in the previous linear regression model unless we had used it to generate data.

### Multi-Variable Linear Regression

Our model accuracy is also suffering from some epistemic uncertainty. Initially we included only size/carats, which we intuited (probably correctly) would be the most important predictor variable. However, there are other useful predictor variables, including depth, cut, color, and clarity. In the app below, observe how the R-squared accuracy metric changes as you include different variables.
...
You should notice that the other variables are helpful in improving the fit of the model. However, some are more useful than others and none are close to the importance of size/carats.
...
Note that this is the traditional form of multi-variate regression where the $\epsilon$ term is constant. Now having seen the data and the equation above, you may be scratching your head as to how the non-numeric values in the data were incorporated into the model.

#### Non-Numeric Inputs

##### Categorical

Generalized Linear Models have the ability to consume non-numeric data, although you may consider it to be not particularly elegant. Effectively we need a parameter for every possible non-numeric data value, such as when cut = Ideal, cut = Premium, or cut = Good. As you can imagine this can lead to a lot of extra parameters, each of which you may call an indicator variable coefficient. Each parameter adjusts the output accordingly if the non-numeric data value is present in the input, otherwise it does nothing.
...
(it'd be really nice to have a plot that shows how, in a vertical sense, the errors are normally distributed or non-normally distributed)

## Probability of the Data

As usual, we started by focusing on the data generated from our models to improve our intution... And now...
