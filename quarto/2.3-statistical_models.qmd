...
## Selecting a Probability Distribution by Conditioning on Data
...
#### Linear Regression Notation Options
...
- The outputs vary from 0 to 1 and can be interpreted as probabilities from a Bernoulli distribution. There is no error term since the output is already interpreted as probabilistic.
...
Here are a couple key elements we want to include and understand if we move away from 'standard' statistical models:
...
As usual, we started by focusing on the data generated from our models to improve our intution. We now consider $P(D|M)$. When writing the standard equation for single variable linear regression, it's hard to understand what 'probability of the data' would even mean:

$$
y_i = \beta_0 + \beta_1 x_i + \epsilon
$$

We should also state explicitly that we've reverted to our initial, or 'standard', version of linear regression with a single variable. However, in our preferred notation, that should become quite a bit more obvious since it's explicit how the data is modeled within a normal/gaussian probability distribution:
...
Due to some poor model assumptions, the probabilities we calculated here do not seem very reasonable. Ultimately this makes P(D|M) non-credible. However, we've also shown some how some fairly simple changes improved our model substantially. We could also calculate $P(D|M)$ with the improved models and get much more reasonable results. 

It's worth noting that with the models shown in this chapter, you could calculate probabilities analytically, as we did above. However, another option is to just simulate a large dataset and check the percentile of a data point of interest. We mention the later again as it may be the only practical option for more realistic problems.