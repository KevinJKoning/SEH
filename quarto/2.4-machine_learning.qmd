...
-   Statistical models are built around probability theory and likelihood
...
There are many machine learning models we could focus on, however, we'll choose Neural Nets, culminating in Bayesian Neural Nets for a few reasons:

A)  They can make probabilistic predictions just like our statistical models
B)  We have already learned the basic building blocks
C)  \[Standard\] Neural nets are the basis of modern AI, a fundamental technology

Comprehending Bayesian Neural Nets from scratch is a loftly goal, so we will tackle it by building up our understanding gradually:

1)  We develop the activation of a single neuron and it's relation to logistic regression
2)  We develop a basic neural net structure
2)  We develop how the neuron activations propogate through the network
...
We are quite unprepared to understand what a full posterior distribution looks like in such a highly-correlated space as neural network weights, but we shoot for the stars. This will motivate us a litte more to understand the second half of the primer.
...
However, the neural net folks prefer the following **standard notation**:
...
The first neuron, N1, performs the following calculation, where $w_n$ is a weight/coefficient, $x$ is the input value (i.e. carats), and $b_n$ is a bias/intercept:

$$
\begin{aligned}
C_1 &= w_{1.1} \cdot x \;+\; b_{1.1}, \\
C_2 &= w_{1.2} \cdot x \;+\; b_{1.2}, \\
C_3 &= w_{1.3} \cdot x \;+\; b_{1.3}.
\end{aligned}
$$
...
graph LR
  ...
  N1 -->|$$C_1 = w_1 \cdot x \;+\; b_1.1$$| N2
  N1 -->|$$C_2 = w_2 \cdot x \;+\; b_1.2$$| N3
  N1 -->|$$C_3 = w_3 \cdot x \;+\; b_1.3$$| N4
```

#### Hidden Layer 1

At Hidden Layer 1, each neuron is only receiving one input, but is creating 3 outputs. Since we are now within the hidden layers, the math is logistic regression instead of linear regression. We examine the calculation occuring at neuron N2. Here $x_1$ is the value received from connection C1.

$$
\begin{aligned}
C_4 &= \frac{1}{1 + e^{-(w_4 \cdot x_1 + b_4)}}, \\
C_5 &= \frac{1}{1 + e^{-(w_5 \cdot x_1 + b_5)}}, \\
C_6 &= \frac{1}{1 + e^{-(w_6 \cdot x_1 + b_6)}}.
\end{aligned}
$$
...
graph LR
  ...
  N2 -->|"$$C_4 = \frac{1}{1 + e^{-(w_4x_1 + b_4)}}$$"| O1
  N2 -->|"$$C_5 =\frac{1}{1 + e^{-(w_5x_1 + b_5)}}$$"| O2
  N2 -->|"$$C_6 =\frac{1}{1 + e^{-(w_6x_1 + b_6)}}$$"| O3
```

#### Hidden Layer 2

At Hidden Layer 2 each neuron takes the values from the first hidden layer of neurons and performs another logistic regression to determine their activation. From the diagram you can see there are lots of connections here. If we examine neuron N5, it is taking in data from connection C4, C7, and C10, and then creating a single output, C13.

$$
C_{13} = \frac{1}{1 + e^{-(w_{13} \cdot x_4 + w_{14} \cdot x_7 + w_{15} \cdot x_10 + b_{13})}}
$$
