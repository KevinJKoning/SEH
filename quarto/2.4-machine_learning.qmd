...
At Hidden Layer 2 each neuron takes the values from the first hidden layer of neurons and performs another logistic regression to determine their activation. If we examine neuron $N^{(2)}_{1}$, it is taking in data from activation $a^{(1)}_{1,1}$, $a^{(1)}_{2,1}$, and $a^{(1)}_{3,1}$, and then creating a single output, $a^{(2)}_{1,1}$.
