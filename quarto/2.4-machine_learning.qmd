...
Although the chapter is titled Machine Learning, we make no attempt to cover the topic broadly. We simply choose a method that fits well in both statistical and machine learning contexts and explore it in detail. The resources for more comprehensive introductions to Machine Learning are easily found elsewhere.

## Bayesian Neural Net

There are many machine learning models we could focus on, however, we'll choose Bayesian Neural Nets for a few reasons:

A) They make probabilistic predictions just like our statistical models
B) We have already learned most of the basic building blocks (of the model, but not the model 'training')
C) [Standard] Neural nets are the basis of modern AI

Comprehending Bayesian Neural Nets from scratch is a loftly goal, we will tackle it by building up our understanding gradually:

1) We develop a 'standard' neural net structure
2) We revisit how each hidden neuron determines its activation via logistic regression
3) Use the 'standard' neural net but add an output to let it determine the variance in the data
4) Use ensembles of the neural nets that can estimate variance
5) Use a Bayesian Neural Net that has a full posterior distribution of the weights

We are quite unprepared to understand what a full posterior distribution looks like in such a highly-correlated space as neural network weights, but we shoot for the stars. This will motivate us a litte more to understand the second half of the primer. Before we layer in the Bayesian piece, we start with 'standard' neural nets.

### Standard Neural Net

 Although there are many variations, we can think of a neural net as layers of artificial neurons that perform a regression-like calculation on the outputs of the previous layer of neurons. An example will make this much more obvious.
...
The first neuron performs the following calculation, where $w_n$ is a weight/coefficient, $x$ is the input value (i.e. carats), and $b_n$ is a bias/intercept:
...
graph LR
  ...
  N1 -->|$$C_1 = w_1 \cdot x \;+\; b_1$$| N2
  N1 -->|$$C_2 = w_2 \cdot x \;+\; b_2$$| N3
  N1 -->|$$C_3 = w_3 \cdot x \;+\; b_3$$| N4
```

#### Hidden Layer 1

At Hidden Layer 1, each neuron is only receiving one input, but is creating 3 outputs. and be activated according to the input value and a weight/coefficient associated with the neuron.

$$
\begin{aligned}
C_4 &= \frac{1}{1 + e^{-(w_4 \cdot x + b_4)}}, \\
C_7 &= \frac{1}{1 + e^{-(w_7 \cdot x + b_7)}}, \\
C_{10} &= \frac{1}{1 + e^{-(w_{10} \cdot x + b_{10})}}.
\end{aligned}
$$
...
graph LR
  ...
  N1 -->|"$$C_4 = \frac{1}{1 + e^{-(w_4x + b_4)}}$$"| O1
  N1 -->|"$$C_7 =\frac{1}{1 + e^{-(w_7x + b_7)}}$$"| O2
  N1 -->|"$$C_{10} =\frac{1}{1 + e^{-(w_{10}x + b_{10})}}$$"| O3
```

#### Hidden Layer 2

At Hidden Layer 2 Each of these neurons take the values from the first layer of neurons and performs another logistic regression to determine their activation. From the diagram you can see there are lots of connections here. N6 will take the activation value from N2 and apply it's own weight/coefficient, same with N3, N4, and N5, and use logistic regression to get it's own activation. That same process repeats with N7, N8, and N9, each with their own weights/coefficients.

Let's show the calcuation that is happening at neuron N5:
...
# Arbitrary weights and bias
w1, w2, w3 = 2.0, 1.0, -1.0
bias = -0.5

def neuron_output(x1, x2, x3):
    """Compute the neuron's output with a logistic activation."""
    z = w1*x1 + w2*x2 + w3*x3 + bias
    return sigmoid(z)

# High and low activation examples
x1_high, x2_high, x3_high = 2, 2, 2
x1_low, x2_low, x3_low = -1, -1, -1

# Print numeric results for the two examples
print("High activation example:")
print(f"  Inputs: ({x1_high}, {x2_high}, {x3_high})")
print(f"  Output: {neuron_output(x1_high, x2_high, x3_high)}\n")

print("Low activation example:")
print(f"  Inputs: ({x1_low}, {x2_low}, {x3_low})")
print(f"  Output: {neuron_output(x1_low, x2_low, x3_low)}\n")

# Plot the activation across a range of x1 values
# We'll fix x2, x3 to the high-activation set for one curve, 
# and fix x2, x3 to the low-activation set for another curve.
x_values = np.linspace(-3, 3, 100)
y_high_curve = [neuron_output(x, x2_high, x3_high) for x in x_values]
y_low_curve  = [neuron_output(x, x2_low,  x3_low)  for x in x_values]

plt.figure(figsize=(8, 5))

# Plot lines for the two fixed-input scenarios
plt.plot(x_values, y_high_curve, label='x2=2, x3=2 (High Activation Curve)')
plt.plot(x_values, y_low_curve,  label='x2=-1, x3=-1 (Low Activation Curve)')

# Mark the specific high- and low-activation points
plt.scatter(x1_high, neuron_output(x1_high, x2_high, x3_high),
            color='red', zorder=5, label='High Activation Point')
plt.scatter(x1_low, neuron_output(x1_low, x2_low, x3_low),
            color='blue', zorder=5, label='Low Activation Point')

plt.title("Neuron Output with Logistic Activation")
plt.xlabel("x1")
...
which is a final regression using the activation of N6, N7, N8, and N9 to get the output value of the neural net.
