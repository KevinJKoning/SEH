...
# Now accepts an 'activation' parameter ("relu" or "logistic") to choose the activation.
class NeuralNetwork:
...
def server(input, output, session):
    ...
    def train_model():
        ...
        # Convert the selected string to lowercase ("ReLu" becomes "relu", "Logistic" becomes "logistic").
        activation = input.activation().lower()
        
        # Create and train the neural network.
        nn = NeuralNetwork(input_size=1, hidden_size1=4, hidden_size2=4, output_size=1, activation=activation)
        epochs = input.epochs()
        learning_rate = 0.01
...
<!-- ```{python}
#| label: bayesian-nn-numpy

# Save standard NN weights as prior means
prior_W1 = W1.copy()
prior_b1 = b1.copy()
prior_W2 = W2.copy()
prior_b2 = b2.copy()

# Re-initialize weights for Bayesian training
...
flatten = lambda arr_list: np.concatenate([a.flatten() for a in arr_list])

for epoch in range(nepochs_bayes):
    z1, a1, z2 = forward_pass(x)
    data_loss = mse_loss(z2, y.reshape(-1,1))
    w_current = flatten([W1_bayes, b1_bayes, W2_bayes, b2_bayes])
    w_prior_current = flatten([prior_W1, prior_b1, prior_W2, prior_b2])
    prior_loss = neg_log_prior(w_current, w_prior_current, prior_variance)
    loss = data_loss + prior_loss / x.shape[0]
