...
The chapter is focused on Bayesian Neural Networks, and although they are an advanced topic, you may be surprised to learn the major building blocks have already been covered. In the flavor we use, each hidden layer neuron is just logistic regression (which is just generalized linear regression) built into a network. Once we build this network we will allow it to generate data like in previous chapters. Because this type of machine learning is probabilistic, we can calculate P(D\|M).
...
We think the best way to understand models is to start using them, i.e. start having them generate data to understand P(D|M), so we built the first half of the primer on that premise. However, an important difference between statistical model and machine learning is how they are trained, which is effectively discussion left for the second half of the primer. Regardless, we'll mention the following as it is an importance difference: 
...
3)  We use the 'standard' neural net but add an output to let it determine the variance in the data
4)  We use ensembles of the neural nets that can estimate variance
5)  We use a Bayesian Neural Net that has a full posterior distribution of the weights
