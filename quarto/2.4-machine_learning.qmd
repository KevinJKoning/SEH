...
The chapter is focused on Bayesian Neural Networks, and although they are a relatively advanced topic, you may be surprised to learn the major building blocks have already been covered. Each neuron is just logistic regression (which is just generalized linear regression) built into a network. We will build a network and then allow it to generate data. Because this type of machine learning is probabilistic, we can calculate P(D|M).

We will also need to visit the topic of overfitting due to the extreme flexibility of neural networks.

## Random Notes, Maybe Delete
...
## Bayesian Neural Network (Simple Implementation)

The difference between a 'standard' neural network and a Bayesian one is that the Bayesian neural network will have uncertainty associated with the parameter values. We will assume this uncertainty follows a normal/gaussian distribution. This uncertainty is key to making this method statistical/probabilistic - otherwise an input will always produce the same output. If the structure of the neural net is appropriate for the model and it has been well trained, a Bayesian neural network should mimic the variation/uncertainty in the outputs.

Bayesian methods require a prior estimate of a parameter value, which will be updated to the extent that it disagrees with new data. (This is a slight oversimplification, we could say that a *good* prior will be updated to the extent it disagrees with new data). We trained a standard neural net so that we would have reasonable prior values.

Here, we’ll add a simple prior on the weights (a Gaussian with user-specified mean and variance) and do a primitive approach to approximate posterior updates.
...
# --- Neural Network Class ---
# (This class is trained in the usual way. After training, we simulate a Bayesian
#  neural network by sampling weight perturbations during prediction.)
class NeuralNetwork:
    def __init__(self, input_size, hidden_size1, hidden_size2, output_size, activation="relu"):
        self.activation = activation  # "relu" or "logistic"
        self.W1 = np.random.randn(input_size, hidden_size1) * np.sqrt(2.0 / input_size)
    ...
    def forward_bayesian(self, X, sigma=0.1):
        # For each layer, add Gaussian noise to the weights and biases to simulate uncertainty.
        W1_sample = self.W1 + np.random.normal(0, sigma, self.W1.shape)
        b1_sample = self.b1 + np.random.normal(0, sigma, self.b1.shape)
        Z1 = np.dot(X, W1_sample) + b1_sample
        if self.activation == "relu":
            A1 = relu(Z1)
        else:
            A1 = logistic(Z1)
        
        W2_sample = self.W2 + np.random.normal(0, sigma, self.W2.shape)
        b2_sample = self.b2 + np.random.normal(0, sigma, self.b2.shape)
        Z2 = np.dot(A1, W2_sample) + b2_sample
        if self.activation == "relu":
            A2 = relu(Z2)
        else:
            A2 = logistic(Z2)
        
        W3_sample = self.W3 + np.random.normal(0, sigma, self.W3.shape)
        b3_sample = self.b3 + np.random.normal(0, sigma, self.b3.shape)
        Z3 = np.dot(A2, W3_sample) + b3_sample
        return Z3
...
# --- Shiny UI ---
app_ui = ui.page_fluid(
    ui.h2("Neural Network Training (Bayesian Simulation)"),
    ui.layout_sidebar(
        ui.sidebar(
            ui.input_slider("epochs", "Number of Training Epochs", min=100, max=5000, value=1000, step=100),
            ui.input_select("activation", "Activation Function", choices=["ReLu", "Logistic"], selected="Logistic"),
...
def server(input, output, session):
    ...
    def train_model():
        ...
        # --- Bayesian Predictions via Monte Carlo Sampling ---
        # Instead of a single deterministic forward pass, we sample the network's prediction
        # multiple times with perturbed weights to simulate uncertainty.
        num_samples = 100
        samples = np.array([nn.forward_bayesian(X_norm, sigma=0.1) for _ in range(num_samples)])
        # Average and standard deviation of predictions (in normalized units)
        y_pred_norm_mean = samples.mean(axis=0)
        y_pred_norm_std = samples.std(axis=0)
        # Convert predictions back to original scale.
        y_pred_mean = y_pred_norm_mean * y_std + y_mean
        y_pred_std = y_pred_norm_std * y_std
        
        return {"losses": losses, "X": X, "y": y, 
    ...
    def predictionPlot():
        ...
        X = result["X"].flatten()
        y = result["y"].flatten()
        y_pred = result["y_pred_mean"].flatten()
        y_std = result["y_pred_std"].flatten()
        
        # Sort the data by X for a clear line plot.
        order = X.argsort()
        X_sorted = X[order]
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.scatter(X, y, color="blue", label="Actual Price")
        ax.plot(X_sorted, y_pred_sorted, color="red", label="Predicted Mean")
        ax.fill_between(X_sorted, y_pred_sorted - 2*y_std_sorted, y_pred_sorted + 2*y_std_sorted,
                        color="red", alpha=0.3, label="Uncertainty (±2σ)")
        ax.set_title("Actual vs. Predicted Diamond Price (Bayesian NN Simulation)")
        ax.set_xlabel("Carat")
        ax.set_ylabel("Price")
        ax.legend()
        return fig
...
# --- Neural Network Class with SGLD ---
# Fixed architecture: 1-4-4-1 with ReLU.
# SGLD update: theta <- theta - (lr/2)*grad + N(0, sqrt(lr))
class NeuralNetwork:
    ...
    def backward(self, X, y_true, y_pred, learning_rate):
        ...
        # --- SGLD updates ---
        self.W3 = self.W3 - (learning_rate/2)*dW3 + np.random.normal(0, np.sqrt(learning_rate), self.W3.shape)
        self.b3 = self.b3 - (learning_rate/2)*db3 + np.random.normal(0, np.sqrt(learning_rate), self.b3.shape)
        self.W2 = self.W2 - (learning_rate/2)*dW2 + np.random.normal(0, np.sqrt(learning_rate), self.W2.shape)
        self.b2 = self.b2 - (learning_rate/2)*db2 + np.random.normal(0, np.sqrt(learning_rate), self.b2.shape)
        self.W1 = self.W1 - (learning_rate/2)*dW1 + np.random.normal(0, np.sqrt(learning_rate), self.W1.shape)
        self.b1 = self.b1 - (learning_rate/2)*db1 + np.random.normal(0, np.sqrt(learning_rate), self.b1.shape)
    
    def train(self, X, y, epochs, learning_rate, burn_in, sample_interval):
        losses = []
        samples = []
        for epoch in range(epochs):
            y_pred = self.forward(X)
            ...
            # Store weight samples after burn-in at specified intervals.
...
# --- Helper: forward pass using a given weight sample ---
...
# --- Shiny UI ---
app_ui = ui.page_fluid(
    ui.h2("Bayesian Neural Network with SGLD"),
    ui.layout_sidebar(
        ui.sidebar(
            ui.input_slider("epochs", "Training Epochs", min=100, max=10000, value=1000, step=100),
            ui.input_slider("learning_rate", "Learning Rate", min=0.0001, max=0.01, value=0.01, step=0.001),
            ui.input_slider("burn_in", "Burn-in Epochs", min=0, max=5000, value=500, step=50),
...
def server(input, output, session):
    ...
    @reactive.Calc
    def train_model():
        # Read and preprocess the diamond dataset.
        df = pd.read_csv(StringIO(data_str))
        X = df["carat"].values.reshape(-1, 1)
        y = df["price"].values.reshape(-1, 1)
        ...
        # Get hyperparameter values from the UI.
        epochs = input.epochs()
        ...
        losses, samples = nn.train(X_norm, y_norm, epochs, learning_rate, burn_in, sample_interval)
        
        # Compute predictions for each stored sample.
        preds = []
        for sample in samples:
            pred_norm = forward_with_sample(X_norm, sample)
            preds.append(pred_norm)
        preds = np.array(preds)  # shape: (num_samples, n_data)
        ...
        return {"losses": losses, "X": X, "y": y, "pred_mean": pred_mean, "pred_std": pred_std}
    ...
    def lossPlot():
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.plot(losses, color="blue")
        ax.set_title("Training Loss over Epochs (SGLD)")
        ax.set_xlabel("Epoch")
        ax.set_ylabel("MSE Loss")
        return fig
    ...
    def predictionPlot():
        ...
        # Sort data by X for a smooth prediction curve.
...
class NeuralNetwork:
    ...
    def backward(self, X, y_true, y_pred, learning_rate):
        ...
        # --- SGLD update ---
        # Update rule: theta <- theta - (lr/2)*grad + N(0, sqrt(lr))
        ...
        self.W3 = self.W3 - (learning_rate/2)*dW3 + np.random.normal(0, learning_rate, self.W3.shape)
        self.b3 = self.b3 - (learning_rate/2)*db3 + np.random.normal(0, learning_rate, self.b3.shape)
        self.W2 = self.W2 - (learning_rate/2)*dW2 + np.random.normal(0, learning_rate, self.W2.shape)
        self.b2 = self.b2 - (learning_rate/2)*db2 + np.random.normal(0, learning_rate, self.b2.shape)
        self.W1 = self.W1 - (learning_rate/2)*dW1 + np.random.normal(0, learning_rate, self.W1.shape)
        self.b1 = self.b1 - (learning_rate/2)*db1 + np.random.normal(0, learning_rate, self.b1.shape)
    ...
    def train(self, X, y, epochs, learning_rate, burn_in, sample_interval):
        ...
        # Optionally, one might decay the learning rate after burn-in.
        lr = learning_rate
        ...
        for epoch in range(epochs):
            ...
            # Collect weight samples after burn-in at intervals.
            if epoch >= burn_in and (epoch - burn_in) % sample_interval == 0:
                sample = {
                    "W1": self.W1.copy(),
                    "b1": self.b1.copy(),
            ...
            # Example of a simple learning rate decay after burn-in:
            if epoch == burn_in:
                lr = learning_rate * 0.1  # reduce the lr after burn-in
        
        return burn_in_losses, sampling_losses, samples

# --- Helper function: forward pass with a given weight sample ---
def forward_with_sample(X, sample):
    Z1 = np.dot(X, sample["W1"]) + sample["b1"]
    A1 = relu(Z1)
    Z2 = np.dot(A1, sample["W2"]) + sample["b2"]
...
app_ui = ui.page_fluid(
    ...
    ui.layout_sidebar(
        ui.sidebar(
            ui.input_slider("epochs", "Training Epochs", min=100, max=10000, value=1000, step=100),
            ui.input_slider("learning_rate", "Learning Rate", min=0.00001, max=0.001, value=0.0001, step=0.00001),
            ui.input_slider("burn_in", "Burn-in Epochs", min=0, max=5000, value=500, step=10),
            ui.input_slider("sample_interval", "Sample Interval", min=1, max=50, value=10, step=1)
        ),
        ui.navset_tab(
...
def server(input, output, session):
    ...
    def train_model():
        ...
        # Create and train network using SGLD.
        nn = NeuralNetwork(input_size=1, hidden_size1=4, hidden_size2=4, output_size=1)
        burn_in_losses, sampling_losses, samples = nn.train(X_norm, y_norm, epochs, learning_rate, burn_in, sample_interval)
        
        # For predictions: get outputs from each collected sample.
        preds = []
        for sample in samples:
            pred_norm = forward_with_sample(X_norm, sample)
            preds.append(pred_norm)
