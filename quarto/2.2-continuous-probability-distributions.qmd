...
It's worth noting that there are many real world problems where values can only take positive values.
...
You may have guessed it's related to the normal distribution in that if you have a set of values from the lognormal distribution, if you take the their log values and replot them, you'll end up plotting values in a normal distribution. 
...
We've been hinting that the probability density function would require some explanation, and we've finally come to the right place to tackle it. We've continued to create histograms of our continuous data, in which we take multiple exact values and lump them together in a bin of the histogram. It's possible to use our relative frequency techinique to estimate the probability of the bin (just divide the bin count by the total count of all bins). However, how would estimate the probability of the point within the bin? The problem is that, as we've noted earlier, probability of any exact value on the real number line $\mathbb{R}$ is effectively zero.
...
Both points are important but we'll use the second one in the immediate future. That each point
