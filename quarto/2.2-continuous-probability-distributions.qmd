...
### A More Formal Definition of Probability Distributions

We have, and will, use intuitive language like 'data generating process' that creates a 'relative distribution of outcomes'. For the sake of giving you an interface to 'real' statistical language, we take a moment to elaborate on *slightly* more formal language (we still will not go to the mathematical depths a true statistical reference would).
