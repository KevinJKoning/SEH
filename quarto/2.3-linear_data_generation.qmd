...
We begin by clarifying data inputs vs model parameters. We then start with the most common linear model, linear regresion. Just like in previous chapters, we focus on generating data to understand our model. This will allow us to better understand the model assumptions.
...
While we avoid 'tuning' our models until the second half of the primer, we will use insights from generating data that allow us to 'trade' for a  better model. We'll be given a model that fits the data well as possible given our model constraints and we'll use it to find the probability of data.

We round out the chapter by observing how simple transformations can allow us to apply the linear model methodology to data that otherwise appears non-linear. We'll use those new models to find the probability of data given the model.
