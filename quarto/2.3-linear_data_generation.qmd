...
There are some interesting features of the data in comparison to the data generated by the linear regression model. But we'd like to point these out first:

-   The observed data at small carat sizes has very little noise/variation
-   The observed data at large carat sizes has a lot of noise/variation
-   The generated data has constant noise/variation across all carat sizes
-   (negative values)
-   (step function by 0.5, 1.0 etc. carat size)
...
The linear regression model of the diamond had this mathematical form:

$$
y_i = \beta_0 + \beta_1 x + \epsilon
$$

Where $\beta_0$ is the y-intercept, $\beta_1$ is the the slope, and $\epsilon$ is the error. The variable x is the diamond size/carat and y is the price.
...
Our model accuracy is suffering from some epistemic uncertainty. Initially we included only size/carats, which we intuited (probably correctly) would be the most important predictor variable. However, there are other useful predictor variables, including depth, cut, color, and clarity... (there can also be not useful variables, and not useful *enough* variables).
    ...
    ui.layout_sidebar(
        ui.sidebar(
            ui.input_checkbox_group(
                "predictors",
                "Select Predictors:",
                choices=["carat", "depth", "table", "x", "y", "z"],
                selected=["carat", "depth"]
            ),
            ui.input_slider("num_points", "Number of Simulated Points", 
                           min=100, max=1000, value=200, step=100),
            ui.input_slider("noise_level", "Noise Level", 
...
def server(input, output, session):
    ...
    def modelSummary():
        ...
        for predictor, coef in model_results['coefficients'].items():
            summary += f"{predictor}: {coef:.2f}\n"
        summary += f"\nIntercept: {model_results['intercept']:.2f}"
