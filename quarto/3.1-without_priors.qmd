...
Performing the likelihood calculation described above hinges on being able to calculate $P(y_i \mid \theta)$. To start, we assume we are using a model of a data generating process where the model is just a named parametric probability distribution. To have an example, let's assume we are measuring the reliability of a similar manufacturing line across several different plants. Over the course of a year, there are a wide variety of unplanned shutdowns, with Line A having 7, Line B having 0, Line C having 2, etc. Across all the plants we want to know the most likely value for the rate of unplanned shutdowns, assuming our data follows a Poisson distribution.

First, we pick candidate model parameters. Based on a brief look at the data we think 5 is a reasonable rate per year. Subsequently, that will be our assumed parameter value ($\lambda = 5$) for a candidate model. To find $P(y_i \mid \lambda = 5)$ for a single data point, we can use the app below which is similar to apps we used in the first half of the primer.

We keep $\lambda = 5$ in the app and try to find $P(y_i=7 \mid \lambda = 5)$. Using the app P = 0.1044. Once we can find $P(y_i \mid \theta)$ for one data point, we can do it for the rest and calculate $\mathcal{L}(\theta \mid \mathbf{y})$. Of course, we have not yet *maximized* that likelihood function. We will find strategies to do that later.
...
To have an example, let's assume we are measuring the same dimension over may parts manufactured on the same line, and we want to find the parameters of a normal/gaussian distribution that best describes the variation.

After taking a brief look at the data we guess a candidate model with normal distribution parameters, $y_i \sim \mathcal{N}(\mu = 4, \sigma^2 = 0.00003)$. Now to find the probability of a single data point, we do the same thing we did in the chapter on Continuous Probability Distributions:
...
app_ui = ui.page_fluid(
    ...
    ui.layout_sidebar(
        ui.sidebar(
            ui.input_slider(
                "muInput", "Mean (μ):",
                min=3, max=5, value=4, step=0.1
            ),
            ui.input_slider(
                "varInput", "Variance (σ²):",
                min=0.00001, max=0.00005, value=0.00003, step=0.00001
            ),
            ui.input_numeric(
                "xInput", "Value (x):",
                value=4.005, min=3.9, max=4.1
            ),
            ui.br(),
...
The discussion above may look fine, but how do we calculate an actual value for the likelihood? Regardless of the probability distribution assumed for the errors we first need to know or estimate the parameters.



**Good place for a chart with the verticall/rotated probability distributions???**
...
and by our model assumption, these errors follow a normal distribution:

$$
\epsilon_i \sim \mathcal{N}(0, \sigma^2).
$$

Which should look familiar. We can now find the Maximum Likelihood Estimate of the model parameters ($\beta_0$, $\beta_1$... $\sigma$) based on the likelihood of our data under the given distribution of errors.
...
If we assume constant variance the first term,  $-\frac{n}{2} \log (2\pi\sigma^2)$, is constant. So we simply need to minimize the second term, which is the **sum of squared errors (SSE)**:

$$
\sum_{i=1}^{n} (x_i - \mu)^2
$$

There are both analytical ($(X^T X)^{-1} X^T Y$) as well as computational methods for minimizing the sum of squared errors.
...
### Maximum Likelihood without Constant Normal Variance

As we did before, we cast off the shackles of restrictive modeling assumptions. Because you are an engineer in a hurry, we skip the long derivation and get to the point. We will need to minimize the following function instead of the sum of squared errors (SSE), which is weighted least squares (WLS):

$$
\sum_{i=1}^{n} \frac{(x_i - \mu)^2}{\sigma_i^2}
$$

In general, however, you don't need to find a simpler function to minimize - as we've done here. Instead, once we assume a probability distribution, like the normal distribution, we can maximize it explicitly:


**NEEDS WORK**
$$
\mathcal{L}(\theta \mid \mathbf{y}) = \prod_{i=1}^{N} \mathcal{N}(y_i - \hat{y}_i, \sigma^2).
$$ 
