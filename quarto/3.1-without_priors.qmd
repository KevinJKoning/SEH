...
*Need somewhere to tackle the creation of variance in Frequentist models vs Bayesian*
...
When building a candidate model, we must assume a probability distribution for the data in order to calculate $P(y_i \mid \theta)$. For example, if we assume a normal (Gaussian) distribution, each data point is modeled as:

$$
y_i = \hat{y}_i + \epsilon_i,\quad \epsilon_i \sim \mathcal{N}(0, \sigma^2)
$$

Alternatively, you can express the error term as the difference between the actual and predicted values:

$$
\epsilon_i = y_i - \hat{y}_i,\quad \epsilon_i \sim \mathcal{N}(0, \sigma^2)
$$

Here:

- $\hat{y}_i$ is the model’s prediction for $y_i$.
- $\sigma^2$ represents the variance of the noise (i.e., the spread of $y_i$ around $\hat{y}_i$).
- $\epsilon_i$ is a normally distributed error term.[^1]

[^1]: An important distinction between the *frequentist* approach described in this chapter and the *Bayesian* one described in the next chapter ("With Priors"), is that in the frequentist approach the only variance/uncertainty from the model comes from the error term. This is in contrast to the Bayesian approach in which the model itself (i.e. the parameters of the model) is considered to be uncertain, in addition to the typical inclusion of an error term.

Put simply, the larger the gaps between the predictions and the data, the smaller the likelihood will be.
...
We start with an example in which the Binominal Distribution is an obvious choice for a model of the data generating process. (Setting the model and focusing on the parameters allows us to ignore the more 'meta' problem of deciding on a model type that fits the problem).
...
Let's breakdown how the likelihood was calculated for *one* of the datapoints before we try to maximize the likelihood over all of the datapoints. 

### Reminder of Binomial Probabilities

Let's remind ourselves how to calculate the probability of an observation that follows the Binomial probability distribution:
...
This notation makes it obvious that our estimate for $y_i$ is conditioned on data; our estimate is still a probability distribution; and that the parameters of our linear regression model are used only to estimate the mean of the probability distribution at the $x_i$ location.

In the first half of the primer we made no attempt to find the 'best' values of the parameters, but now we have the tool, specifically Maximum Likelihood Estimation, to do this. We can find the Maximum Likelihood Estimate of the parameters ($\beta_0$, $\beta_1$... $\sigma$) of a traditional linear regression model (with constant variance) either through the computational techniques given earlier, or known analytic solutions.

**Reminder of the app at the end of 2.3 for linear regression**
**Work on that but with the squares as residuals, and minimized the area of the squares**

...While any method, computational or analytical, can lose sight of important details, analytical methods are often forced to make simplifying assumptions 'under the hood' in order find tractable solutions.

We walk through the analytical derivation of linear regression specifically to highlight these simplifying assumptions.

### Linear Regression Analytical Solution Derivations (REMOVE)

Here we reproduce the 'standard' methodology for computing the parameters of a linear regression model. Single-variable linear regression aims to model the relationship between a single independent variable $x$ and a dependent variable $y$ using a linear equation:

We begin with the classical formulation:
$$
y_i = \beta_0 + \beta_1 x_i + \epsilon_i,
$$

where $\beta_0$ is the intercept, $\beta_1$ the slope, and $\epsilon_i$ the error term. The classical form implies a deterministic component $\beta_0+\beta_1 x_i$ with random deviations $\epsilon_i$, typically assumed to have"

$$
\epsilon_i \sim \mathcal{N}(0, \sigma^2).
$$

We've previosly stated we prefer the following notation where we condition on the input:
$$
P(y_i \mid x_i) \sim \mathcal{N}(\beta_0 + \beta_1 x_i, \sigma^2).
$$

This notation emphasizes that the distribution of $y_i$ is centered around $\beta_0 + \beta_1 x_i$ with constant variance $\sigma^2$, and that our predictions are explicitly conditioned on the observed $x_i$.

---

### Derivation of the Analytical Solution

The objective is to find $\beta_0$ and $\beta_1$ that minimize the sum of squared errors (SSE):
$$
...
$$
\bar{y} = \beta_0 + \beta_1 \bar{x} \quad \Longrightarrow \quad \beta_0 = \bar{y} - \beta_1 \bar{x},
$$
...
Substitute $\beta_0 = \bar{y} - \beta_1 \bar{x}$ into the above:
$$
\sum_{i=1}^{n} x_i y_i = \beta_0 \sum_{i=1}^{n} x_i + \beta_1 \sum_{i=1}^{n} x_i^2.
$$

Replacing $\beta_0$ and rearranging, we eventually obtain:
$$
...
Substituting $\beta_1$ back into the equation for $\beta_0$:
$$
...
The error term $\epsilon_i$ represents the random variation in the response not captured by the deterministic part of the model. Under the assumption:
$$
\epsilon_i \sim \mathcal{N}(0, \sigma^2),
$$

we have:
- **Unbiasedness:** $\mathbb{E}[y_i] = \beta_0 + \beta_1 x_i$, meaning that on average our model correctly predicts $y_i$.
- **Constant Variance:** The homoscedasticity assumption ($\sigma^2$ is constant) justifies minimizing the sum of squared errors, which is equivalent to maximizing the likelihood when $y_i \sim \mathcal{N}(\beta_0 + \beta_1 x_i, \sigma^2)$.

In a maximum likelihood framework, the log-likelihood for the observed data is:
$$
\log L(\beta_0, \beta_1, \sigma^2) = -\frac{n}{2} \log(2\pi\sigma^2) - \frac{1}{2\sigma^2} \sum_{i=1}^n \left(y_i - \beta_0 - \beta_1 x_i\right)^2.
$$

Maximizing this log-likelihood with respect to $\beta_0$ and $\beta_1$ leads to the same normal equations as minimizing the SSE.

---

### Summary

The analytical solutions to the linear regression problem are:
$$
\beta_1 = \frac{\sum_{i=1}^{n} (x_i - \bar{x})(y_i - \bar{y})}{\sum_{i=1}^{n} (x_i - \bar{x})^2} \quad \text{and} \quad \beta_0 = \bar{y} - \beta_1 \bar{x}.
$$

This derivation demonstrates how the inclusion of the error term $\epsilon_i$ in the model acknowledges the inherent variability in the data and justifies the least squares approach, which under normal error assumptions, is equivalent to the maximum likelihood estimation.
...
Regardless, we stop a moment to contemplate the need for these *Bayesian* methods. 'Priors' refers to having prior information (or beliefs) about the parameter values. These are a necessary part of the Bayesian framework, can be vague/non-commital or specific, and depending on the situation can be very useful or just invite additional questions on the model accuracy. These points are long discussed in statistical literature.

The point I would like to make here is that once you get past some reasonable starting point for the prior parameter values, you open up an extremely attractive computational framework. In this framework the parameter values are given a probabilistic interpretation - you do not simply find a single most likely value at the top of the hill - you find a range of parameter values with different plausibility. This is much more the reality of the situation.

Also, the methods used end up being *extremely general*. You can define all sorts of custom models, and a single Bayesian sampling method will crank through them all no-sweat giving you plausible models.

But again, we wait a chapter.

## Machine Learning

We again visit machine learning, but will now have the needed background/terminology for distinguishing it from the statistical models we've otherwise discussed.
