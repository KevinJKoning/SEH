...
We finally flip to $P(D|M)$ the probability of a [model]{style="text-decoration: underline;"} given the [data]{style="text-decoration: underline;"}. In practice, we find $P(D|M)$ so that we can find the *best* parameter values for our chosen model. We will describe what *best* is soon.
