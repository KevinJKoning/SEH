...
When building a candidate model, you must assume a probability distribution for the data in order to calculate $P(y_i \mid \theta)$. For example, if you assume a normal (Gaussian) distribution, each data point is modeled as:
...
We start with an example in which the Binominal Distribution is an obvious choice for a model of our data generating process. (Setting the model and focusing on the parameters allows us to ignore the more 'meta' problem of deciding on a model that fits the problem).
...
Yopu may be asking yourself - what is the difference between what we are doing here and what we did in the first half of the primer when we calculated P(D|M)?
...
## Assumptions in Analytical Solutions

So far we've focused on using computational methods to find the Maximum Likelihood. For a subset of problems there are also analytical solutions. While any method, computational or analytical, can lose sight of important details, analytical methods are often forced to make simplifying assumptions 'under the hood' in order find tractable solutions.
