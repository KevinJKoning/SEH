...
A visual here can be very helpful. For each data point we calculate like the probabilities like in the chart below, where every observation has a 'sideways' normal distribution centered at the prediction, and the probability of the point is based on how far out it would land on that normal distribution.
...
A loss function, denoted as $L$, is typically computed as the difference between the true target values $y$ (it's machine learning so we say target instead of observed...) and the predicted values $\hat{y}$. The generic form of a loss function can be expressed as:
...
**add a third tab which gives the parameter values**
