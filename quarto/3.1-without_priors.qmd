...
There are both analytical ($\hat{\beta} = (X^T X)^{-1} X^T Y$) as well as computational methods for minimizing the sum of squared errors.
...
As we did before, we cast off the shackles of restrictive modeling assumptions. These restrictive assumptions allow for nice analytic solutions - which made sense when all the calculation was by hand - but is not nearly so relevant today.
