...
First, we pick candidate model parameters. Based on a brief look at the data we think 5 is a reasonable rate per year, so $\lambda = 5$ for our candidate model. To find $P(y_i \mid \lambda = 5)$ for a single data point, we can use the app below which is similar to apps we used in the first half of the primer.
