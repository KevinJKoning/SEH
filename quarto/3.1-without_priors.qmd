...
Alternatively we could also replace $y_i$ with $d_i$ for a single data point: $\mathcal{L}(\theta \mid D) = \prod_{i=1}^{N} P(d_i \mid \theta)$
...
This approach is excellent even for many parameters, so we jump right to the definition for multi-parameter spaces. We seek to compute the gradient of a likelihood function $L(\theta)$ with respect to parameter[s] $\theta$ so we can use it in an optimization algorithm:
...
- $\theta^{(t)}$ is your current estimate of the parameters at iteration t.
- $\nabla L(\theta^{(t)})$ is the gradient of the likelihood function evaluated at $\theta^{(t)}$, it tells you the direction in which $L(\theta)$ increases the most.
- $\eta$ is the learning rate, a small positive number that controls the size of your step.
...
x_obs2 = 7
y_obs2 = 2 * x_obs2 + 1 - 1  # Modified error from -2 to -1

sigma = 0.8  # Standard deviation
scale_factor = 1.5  # Increase the scale of the sideways normal distributions

# For first point
mu1 = 2 * x_obs1 + 1  # Mean at regression line
y_norm1 = np.linspace(mu1 - 3*sigma, mu1 + 3*sigma, 100)
pdf1 = norm.pdf(y_norm1, mu1, sigma)
x_norm1 = pdf1 / np.max(pdf1) * scale_factor + x_obs1  # Scale and shift

# For second point
mu2 = 2 * x_obs2 + 1  # Mean at regression line
y_norm2 = np.linspace(mu2 - 3*sigma, mu2 + 3*sigma, 100)
pdf2 = norm.pdf(y_norm2, mu2, sigma)
x_norm2 = pdf2 / np.max(pdf2) * scale_factor + x_obs2  # Scale and shift
...
# Draw vertical lines from points to regression line (showing the residual error)
...
# --- Additional annotations to highlight error calculation ---
# For the first point, mark the corresponding PDF value at y_obs1
pdf_obs1 = norm.pdf(y_obs1, mu1, sigma)
x_pdf_obs1 = pdf_obs1 / np.max(pdf1) * scale_factor + x_obs1
plt.scatter(x_pdf_obs1, y_obs1, color='red', s=50, zorder=3)
plt.plot([x_pdf_obs1, x_obs1], [y_obs1, y_obs1], 'r--', linewidth=1)
plt.annotate(f'Error = {y_obs1-mu1:.2f}',
             xy=(x_pdf_obs1, y_obs1), xytext=(x_pdf_obs1+0.5, y_obs1),
             arrowprops=dict(arrowstyle="->", color='red'),
             color='red')

# For the second point, mark the corresponding PDF value at y_obs2
pdf_obs2 = norm.pdf(y_obs2, mu2, sigma)
x_pdf_obs2 = pdf_obs2 / np.max(pdf2) * scale_factor + x_obs2
plt.scatter(x_pdf_obs2, y_obs2, color='purple', s=50, zorder=3)
plt.plot([x_pdf_obs2, x_obs2], [y_obs2, y_obs2], 'purple', linestyle='--', linewidth=1)
plt.annotate(f'Error = {mu2-y_obs2:.2f}',
             xy=(x_pdf_obs2, y_obs2), xytext=(x_pdf_obs2+0.5, y_obs2),
...
plt.title("Linear Regression with Enhanced Normal Distributions of Error", fontsize=14)
