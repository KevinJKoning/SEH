...
We're also going to include some rolls with a cup of 3 weighted dice. The values and probabilities will be 1=0.10, 2=0.10, 3=0.15, 4=0.15, 5=0.2, 6=0.30. If we expect the outcomes of fair dice, we'll see that the weighted dice will usually trend towards being a fairly inprobable series of rolls. We can refer to the percentile of their log-probability to see exactly how unusual they are.

**Older text to integrate or delete**
...
def roll_three_dice_weighted():
    ...
    # Define weights: 1=0.15, 2=0.15, 3=0.15, 4=0.15, 5=0.2, 6=0.2
...
def get_probability_of_sum_weighted(dice_sum):
    """Calculate probability of getting a specific sum with three weighted dice."""
    weights = [0.10, 0.10, 0.15, 0.15, 0.2, 0.30]
    outcomes = [1, 2, 3, 4, 5, 6]
    # Generate all possible outcomes with their probabilities
    from itertools import product
    probabilities = {}
    for a, b, c in product(outcomes, repeat=3):
        s = a + b + c
        prob = weights[a-1] * weights[b-1] * weights[c-1]
        probabilities[s] = probabilities.get(s, 0) + prob
    return probabilities.get(dice_sum, 0)
...
app_ui = ui.page_fluid(
    ...
    ui.layout_sidebar(
        ui.sidebar(
            ...
            ui.output_ui("selectedDetails"),  # Changed from output_text to output_ui
            ui.hr(),
            # Removed the Number of Simulations slider
            # Set Number of Simulations to 100 for fair and 5 for weighted
            # Retain Number of Rolls slider
...
def server(input, output, session):
    ...
    def selectedDetails():
        ...
        selected = input.selectedSim()
        if not selected:
            return ui.p("No simulation selected.")
        try:
            # Find the simulation by label
            selected_sim = next((sim for sim in sims if sim[0] == selected), None)
            if not selected_sim:
                return ui.p("Selected simulation not found.")
            label, cum_probs = selected_sim
            # Get the final cumulative log probability
            final_log_prob = cum_probs[-1]
            # Collect all final cumulative log probabilities
            ...
            return ui.div(
                ui.h4(f"{label}"),
                ui.p(f"<strong>Final Cumulative Log10 Probability:</strong> {final_log_prob:.4f}"),
                ui.p(f"<strong>Percentile:</strong> {percentile:.2f}th")
            )
        except (IndexError, ValueError):
            return ui.p("Invalid selection.")
...
- Most people do not understand the *null* models and what is truly a less than 0.05 probability
- 0.05 is the traditional value, but hardly makes sense in many circumstances.
